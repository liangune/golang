...
func (l *LoggingT) header(s Severity, depth int) (*buffer, string, int) {
	_, file, line, ok := runtime.Caller(calldepath + depth)
	if !ok {
		file = "???"
		line = 1
	} else {
...
func (l *LoggingT) createFiles(sev Severity) error {
	...
	*/
	// file is created in current severity, not in decreasing severity order
	if s := sev; s >= debugLog && l.file[s] == nil {
		sb := &syncBuffer{
			logger: l,
			sev:    s,
		}
...
func (l *Logger) NoticeDepth(depth int, args ...interface{}) {
	l.l.printDepth(noticeLog, depth, args...)
}

func (l *Logger) InterfaceAverageDurationDepth(depth int, args ...interface{}) {
	l.l.printDepth(interfaceAverageDurationLog, depth, args...)
}

func (l *LoggingT) LogFileMode(mode os.FileMode) *LoggingT {
	l.logFileMode = mode
	return l
}

func (l *Logger) LogFileMode(mode os.FileMode) {
	l.l.logFileMode = mode
}
